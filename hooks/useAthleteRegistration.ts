...
export function useAthleteRegistration({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedData) {
        ...
        console.log('📚 Parsed athlete data from localStorage:', parsedData);
        ...
        if (parsedData.address === currentAddress && parsedData.chainId === chainId) {
          ...
          console.log('✅ Athlete data recovered and applied');
        } else {
          console.log('⏭️ Saved data does not match current wallet/network, skipping recovery');
  ...
  useEffect(() => {
    console.log("🔍 FHEVM instance status (useAthleteRegistration):", {
    ...
    if (fhevmStatus === 'error' && !fhevmInstanceParam) {
      console.error("❌ Internal FHEVM initialization failed in useAthleteRegistration");
    }
  ...
  const contractInfo: AthleteRegistrationInfoType = useMemo(() => {
    ...
    console.log("🔍 getAthleteRegistrationInfo result:", info);
    console.log("🔍 AthleteRegistrationABI type:", typeof AthleteRegistrationABI);
    console.log("🔍 AthleteRegistrationABI has abi property:", !!AthleteRegistrationABI.abi);
    console.log("🔍 AthleteRegistrationABI.abi is array:", Array.isArray(AthleteRegistrationABI.abi));
    console.log("🔍 AthleteRegistrationABI.abi length:", Array.isArray(AthleteRegistrationABI.abi) ? AthleteRegistrationABI.abi.length : 'not array');
    console.log("🔍 AthleteRegistrationABI.abi sample:", Array.isArray(AthleteRegistrationABI.abi) ? AthleteRegistrationABI.abi.slice(0, 2) : 'not array');
    return info;
  }, [chainId]);
  ...
  const contract: ethers.Contract | undefined = useMemo(() => {
    if (contractAddress === undefined) {
      console.log("🔍 Contract creation skipped - missing address");
      return undefined;
    }
    ...
    if (!canCreateContract) {
      console.log("🔍 Contract creation skipped - missing address or signer on non-local network");
      return undefined;
    }
    ...
    try {
      ...
      if (isLocalNetwork && !ethersSigner) {
        ...
        providerOrSigner = new JsonRpcProvider("http://127.0.0.1:8545");
      }
  ...
  const refreshAthleteInfo = useCallback(async () => {
    ...
    if (!canRefreshBasic) {
      console.log("🔄 Cannot refresh athlete info - basic conditions not met");
    ...
    try {
      ...
      // For local network (chainId 31337), prioritize localStorage over contract calls
        if (chainId === 31337) {
        console.log("🔍 Local network - prioritizing localStorage over contract calls");
          ...
          try {
            ...
            if (stored) {
              ...
              if (parsed.address === currentAddress && parsed.chainId === chainId) {
                foundInStorage = true;
              console.log("🔍 Found stored data in localStorage for local network");
              // Set athlete info from localStorage if available
              if (parsed.encryptedName) {
                console.log("🔍 Athlete is registered, setting athlete info...");
        ...
        if (!registered) {
          try {
            console.log("🔍 Local network - trying to get data from contract as fallback");
        ...
        try {
          ...
          console.log("🔍 Assuming not registered due to contract call failure");
          registered = false;
        }
        setIsRegistered(registered);
      }
      ...
      if (registered) {
        ...
        if (chainId === 31337) {
          ...
          console.log("🔍 Local network - trying to get data from contract");
          ...
          if (!athleteInfo && contract) {
            try {
              console.log("🔍 Trying to get data from contract...");
              ...
              if (decryptedData && decryptedData[4]) { // decrypted flag
                console.log("🔍 Got decrypted data from contract:", decryptedData);
                const contractData = {
          ...
          if (!athleteInfo) {
            console.log("🔍 athleteInfo not found, this might be after page refresh - checking if we have decrypted data to recover from");

            // Check if we have previously decrypted data we can use to reconstruct athleteInfo
            if (clearAthleteInfo) {
              console.log("🔍 Found clearAthleteInfo, reconstructing athleteInfo from decrypted data");
              setAthleteInfo({
              ...
              console.log("🔍 No decrypted data found, creating default test data for local network");
              const defaultInfo = {
              ...
              console.log("🔍 Setting default athleteInfo:", defaultInfo);
              setAthleteInfo(defaultInfo);
            }
          } else {
            console.log("🔍 athleteInfo found, keeping existing data");
          }
        } else {
    ...
    try {
        ...
        if (isLocalNetwork) {
            ...
            try {
                ...
                console.log('💾 Plain text athlete data saved to localStorage for address:', signerAddress);
            } catch (error) {
            ...
            const athleteData = {
                ...
                nameHandle: hexlify(encryptedNameInput.handles[0]),
                ageHandle: hexlify(encryptedAgeInput.handles[0]),
                contactHandle: hexlify(encryptedContactInput.handles[0]),
                categoryHandle: hexlify(encryptedCategoryInput.handles[0]),
            };
            ...
            try {
                ...
                console.log('💾 FHE encrypted athlete data saved to localStorage');
    } catch (error) {
                console.warn('Failed to save athlete data to localStorage:', error);
            }
  ...
  useEffect(() => {
    console.log("🏁 AthleteRegistration hook mounted");
